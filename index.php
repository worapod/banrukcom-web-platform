...
if (isset($_GET['page']) && $_POST['page'] == "Auth") {
    $page = decode256_distination($_GET['page']) ?? 'login';
} else {
    $page = $_GET['page'] ?? 'login';
}

if (isset($_GET['SOS']) && isset($_POST['SOS'])) {
    /// --- กำลังวางแผนพัฒนา API REST
    $action = $_GET['SOS'] ?? 'login';
    $page = decode256_distination($_POST['SOS']) ?? 'login';
}
     ...
     case 'contaxt':
        if (isset($_SESSION['user_id'])) { // ---- ตรวจสอบ สิทธิและใช้ MENUADMINS
            if($_SESSION['role']==="Admin"){
                    ...
                    $controller->login_api_rest();    
            
                if ($action === 'get_locations_excluding') {
                        // Check if logged in (optional but recommended for API)
